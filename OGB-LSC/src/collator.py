...
class Batch():
    def __init__(self, idx, attn_bias, attn_edge_type, rel_pos, all_rel_pos_3d_1, in_degree, out_degree, x, edge_input, y, bcc_shortest_dis):
...
def collator(items, max_node=512, multi_hop_max_dist=20, rel_pos_max = 20):
    ...
    items_ = [(item.idx, item.attn_bias, item.attn_edge_type, item.rel_pos, item.in_degree,
        item.out_degree, item.x, item.edge_input[:, :, :multi_hop_max_dist, :], item.y, item.bcc_shortest_dis) for item in items]
    idxs, attn_biases, attn_edge_types, rel_poses, in_degrees, out_degrees, xs, edge_inputs, ys, bcc_shortest_dises = zip(*items_)
