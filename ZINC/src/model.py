...
class Graphormer(pl.LightningModule):
    def __init__(
        ...
        encoders = [EncoderLayer(hidden_dim, ffn_dim, dropout_rate, attention_dropout_rate, num_heads, i)
    ...
    def forward(self, batched_data, perturb=None):
        ...
        inner_rel_pos = batched_data.inner_rel_pos
...
tot_dis = [7, 7, 5, 5, 3, 3, 2, 2, 1, 1, 0, 0]
class MultiHeadAttention(nn.Module):
    def __init__(self, hidden_size, attention_dropout_rate, num_heads, cur_layer):
...
class EncoderLayer(nn.Module):
    def __init__(self, hidden_size, ffn_size, dropout_rate, attention_dropout_rate, num_heads, cur_layer):
        super(EncoderLayer, self).__init__()

        self.self_attention_norm = nn.LayerNorm(hidden_size)
        self.self_attention = MultiHeadAttention(
            hidden_size, attention_dropout_rate, num_heads, cur_layer)
        self.self_attention_dropout = nn.Dropout(dropout_rate)

        self.ffn_norm = nn.LayerNorm(hidden_size)
        self.ffn = FeedForwardNetwork(hidden_size, ffn_size, dropout_rate)
        self.ffn_dropout = nn.Dropout(dropout_rate)
        self.rel_pos_encoder = nn.Embedding(15, num_heads, padding_idx=0)
        # self.bcc_shortest_dis_encoder = nn.Embedding(5, num_heads, padding_idx=0)
